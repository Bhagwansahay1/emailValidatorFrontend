...
export function BlacklistedDomainsTable({ onEdit, onDelete, onAdd }: BlacklistedDomainsTableProps) {
  ...
  const fetchDomains = async (page: number, search?: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/v1/admin/getBlacklistedDomains?page=${page}&limit=50${searchParam}`,
        {
          headers: {
